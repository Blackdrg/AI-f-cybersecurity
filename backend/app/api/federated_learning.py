...
async def receive_federated_update(update: FederatedUpdate, user: dict = Depends(require_admin)):
    ...
    total_updates = await db.fetchval("SELECT COUNT(*) FROM federated_updates WHERE device_id = $1", update.device_id)
    if total_updates >= 5:  # Aggregate every 5 updates
        await aggregate_gradients(db, update.device_id)

    return {"message": "Federated update received", "update_id": update_id}


async def aggregate_gradients(db, device_id):
    updates = await db.fetch("SELECT model_gradients, num_samples FROM federated_updates WHERE device_id = $1 ORDER BY timestamp DESC LIMIT 5", device_id)
    ...
    # Clear old updates
    await db.execute("DELETE FROM federated_updates WHERE device_id = $1", device_id)
